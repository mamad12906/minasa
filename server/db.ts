...
export async function initDB() {
  ...
  // === SaaS multi-tenancy migration (Phase 1, 2026-04-25) ===
  // Adds a tenants table and a tenant_id FK to every per-customer table.
  // All existing rows are backfilled into tenant_id=1 ("Minasa Default") so
  // the current single-tenant deployment keeps working untouched. Future
  // signups create new tenant rows; route handlers will start filtering by
  // req.user.tenant_id in Phase 2.
  await pool.query(`
    ...
    VALUES (1, 'Minasa Default', 'default', 'unlimited', 'active')
    ...
    ALTER TABLE users      ADD COLUMN IF NOT EXISTS tenant_id INTEGER NOT NULL DEFAULT 1 REFERENCES tenants(id);
    ALTER TABLE customers  ADD COLUMN IF NOT EXISTS tenant_id INTEGER NOT NULL DEFAULT 1 REFERENCES tenants(id);
    ALTER TABLE reminders  ADD COLUMN IF NOT EXISTS tenant_id INTEGER NOT NULL DEFAULT 1 REFERENCES tenants(id);
    ALTER TABLE invoices   ADD COLUMN IF NOT EXISTS tenant_id INTEGER NOT NULL DEFAULT 1 REFERENCES tenants(id);
    ALTER TABLE audit_log  ADD COLUMN IF NOT EXISTS tenant_id INTEGER NOT NULL DEFAULT 1 REFERENCES tenants(id);
    ALTER TABLE platforms  ADD COLUMN IF NOT EXISTS tenant_id INTEGER NOT NULL DEFAULT 1 REFERENCES tenants(id);
    ALTER TABLE categories ADD COLUMN IF NOT EXISTS tenant_id INTEGER NOT NULL DEFAULT 1 REFERENCES tenants(id);
    ALTER TABLE ministries ADD COLUMN IF NOT EXISTS tenant_id INTEGER NOT NULL DEFAULT 1 REFERENCES tenants(id);
    ...
    -- The old global UNIQUE constraints (username, lookup names) prevent two
    -- tenants from each having an "admin" user or an identically named
    -- platform. Drop them and recreate as composite (tenant_id, ...).
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS uq_platforms_tenant_name      ON platforms(tenant_id, name);
    CREATE UNIQUE INDEX IF NOT EXISTS uq_categories_tenant_name     ON categories(tenant_id, name);
    CREATE UNIQUE INDEX IF NOT EXISTS uq_ministries_tenant_name     ON ministries(tenant_id, name);

    -- settings PK was just (key) — make it (tenant_id, key) for full isolation.
    ALTER TABLE settings ADD COLUMN IF NOT EXISTS tenant_id INTEGER NOT NULL DEFAULT 1 REFERENCES tenants(id);
    DO $$
    BEGIN
      IF EXISTS (
        ...
        WHERE schemaname = current_schema() AND indexname = 'settings_tenant_key_pkey'
      ) THEN
